function [F_cost] = steady_level_flight_trim(XXX)
alpha_tr = XXX(1) ;
dele_tr = XXX(2) ;
delt_tr = XXX(3) ;
...
%% COST FUNCTION FOR GIVEN INPUTS
...
F_cost = ((F_thrust*cos(alpha_tr)-F_drag)^2 + (F_lift+F_thrust*sin(alpha_tr)-weight_aircraft)^2 + sqrt(M_pitch^2)) ;
% F_cost = norm((F_thrust*cos(alpha_tr)-F_drag)^2 + (F_lift+F_thrust*sin(alpha_tr)-weight_aircraft)^2);
